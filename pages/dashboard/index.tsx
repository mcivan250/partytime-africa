...
const DashboardPage: React.FC = () => {
  ...
  const EventCard = ({ event }: { event: Event }) => {
    ...
    const dateInfo = formatDate(event.date_time || new Date().toISOString());
    ...
    return (
      <Link href={`/events/${event.id}`} className="block">
        <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 cursor-pointer">
          <div className={`bg-gradient-to-br ${themeClasses[event.theme as keyof typeof themeClasses] || themeClasses.sunset} h-32 flex items-center justify-center text-center p-4 relative`}>
